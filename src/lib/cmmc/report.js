...
export function generatePrintableReport(result, gaps, otWarnings, poamDetails, profile) {
  ...
  const familyRows = CONTROL_FAMILIES.map(f => {
    ...
    return `<tr>
      <td style="font-family:monospace;color:#8B6F47">${f}</td>
  ...
  const gapRows = gaps.map(g => `<tr>
    ...
    <td style="font-family:monospace;color:#8B6F47">${g.controlId}</td>
    <td>${g.title}</td>
  ...
  const otSection = otWarnings.length > 0 ? `
    ...
    <table>
      ...
      <tbody>
        ${otWarnings.map(w => `<tr>
          <td style="font-family:monospace;color:#8B6F47">${w.controlId}</td>
          <td>${w.title}</td>
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 32px; padding-bottom: 20px; border-bottom: 3px solid #8B6F47; }
